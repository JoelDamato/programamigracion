...
async function processCSV() {
  ...
  try {
    const fileContent = fs.readFileSync("ConResp.csv", "utf8");
      ...
      complete: async (results) => {
        ...
        results.data.forEach((row, index) => {
          ...
          const phone =
            ...
            const sendDate = row["F. Envio"]?.trim() || null;
            const observation = row["Observaciones"]?.trim() || null;
      ...
      const response = await axios.post(
        "https://api.notion.com/v1/pages",
        {
          ...
          properties: {
            ...
            "Tipo contacto": { select: { name: "Primer contacto" } },
            "Estado interaccion": { select: { name: "Finalizada" } },
            "Origen": { select: { name: data.origin } },
            "Metricas": { relation: [{ id: metricsId }] },
            ...(data.observation && { "Observacion": { rich_text: [{ text: { content: data.observation } }] } }),
          },
        },
        {
          headers: {
