      ...
      complete: async (results) => {
        ...
        for (const client of cleanData) {
          ...
          if (!clientId) {
            ...
            if (newClientId) {
              await createThreeInteractions(client, newClientId);
            }
          } else {
            await createThreeInteractions(client, clientId);
          }
        }
...
// Función para crear tres interacciones con propiedades editables
async function createThreeInteractions(data, clientId) {
  const interactions = [
    {
      properties: {
        ...
        "Tipo contacto": { select: { name: "Generada por Usuario" } },
    ...
    },
    {
      properties: {
        "Nombre cliente": { relation: [{ id: clientId }] },
        "Canal": { select: { name: "Llamada" } },
        "Respuesta": { select: { name: "Respondio" } },
        "Tipo contacto": { select: { name: "Call Confirm" } },
        "Estado interaccion": { select: { name: "Finalizada" } },
        "Origen": { select: { name: data.origin } },
        "Producto de interes": { multi_select: [{ name: "MEG" }] },
        "Aplica?": { select: { name: "Aplica"} },
    ...
    },
    {
      properties: {
        ...
        "Canal": { select: { name: "Llamada" } },
        "Respuesta": { select: { name: "Respondio" } },
        "Tipo contacto": { select: { name: "Llamada MEG" } },
        "Estado interaccion": { select: { name: "Finalizada" } },
        "Origen": { select: { name: data.origin } },
        "Producto de interes": { multi_select: [{ name: "MEG" }] },
        "Metricas": { relation: [{ id: metricsId }] },
        "Asistio?": { select: { name: "Asistió"} },
        "Grabacion Llamada": {
          files: [
            {
              name: "Ejemplo Llamada",
              external: { url: "https://www.example.com/llamada.mp3" },
            },
          ],
        },
      },
  ...
  for (const interaction of interactions) {
    await createInteraction(interaction.properties);
  }
}

// Función para crear una interacción en Notion
async function createInteraction(properties) {
  try {
    const response = await axios.post(
      "https://api.notion.com/v1/pages",
      {
    ...
    console.log(`Interacción creada en Notion (ID: ${response.data.id})`);
  } catch (error) {
